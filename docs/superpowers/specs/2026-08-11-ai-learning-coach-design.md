...
「本單元弱單字練習」預設建立 5 題聚焦佇列，依近期低保持率、提示依賴、慢回想與混淆訊號排序，混合日文→中文、中文→日文、例句填空及混淆對比題。AI 只負責從已驗證的候選單字中選題與解釋；每題仍走既有作答紀錄與 FSRS rating 流程。完成摘要需比較本次表現並列出仍需複習的單字。
...
相同資料每日最多重新分析一次；使用者可手動重新分析，但需有冷卻時間。聊天問題不與每日分析共用快取。
